...
class HalfAPIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path in {"/api/status", "/api/get_pipeline_status"}:
            ...
            self._json_response({"status": "error", "message": f"Unknown: {self.path}"}, 404)
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/api/approve_deployment":
            ...
            if len(signature) >= 8:
                import os
                gate_data = {
                    "status": "approved", "signature": signature,
                    "locked": False, "mrp_ready": True, "deployment_approved": True,
                }
    ...
    def _process_pda_command(self, message: str) -> str:
        ...
        if "run phase" in msg_lower or "execute" in msg_lower:
            ...
            if match:
                ...
                try:
                    ...
                    return f"Phase {match.group(1)} execution: {r.get('status', 'done')}."
                except Exception as e:
        ...
        if "deploy" in msg_lower or "approve" in msg_lower:
            return ("Deployment requires cryptographic sign-off at the Finality Gate.\n"
    ...
    def _get_pipeline_jobs(self) -> list[dict[str, object]]:
        """Get pipeline jobs."""
        return [{"id": "job-1", "name": f"Phase {i}", "status": "pending"} for i in range(1, 6)]

    def _json_response(self, data: dict[str, object] | list[dict[str, object]], status: int = 200) -> None:
