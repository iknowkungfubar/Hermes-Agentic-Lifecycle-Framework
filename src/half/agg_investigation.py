...
class AggressiveInvestigator:
    ...
    def investigate(
        self, failure_description: str, context: dict[str, Any] | None = None
    ) -> InvestigationReport:
        ...
        """
        logger.info(
            "Investigation: Starting investigation of: %s", failure_description[:100]
        )
    ...
    def _check_logs(
        self, report: InvestigationReport, context: dict[str, Any] | None
    ) -> None:
            ...
            for log_file in sorted(log_dir.glob("*"))[:5]:
                try:
                    ...
                    error_lines = [
                        ...
                        if any(
                            kw in l.upper()
                            for kw in ["ERROR", "CRITICAL", "FAILED", "TRACEBACK"]
                        )
                    ]
                    if error_lines:
                        report.steps.append(
    ...
    def _write_debug_script(
        ...
        if "import" in failure or "module" in failure:
            ...
            for word in failure.split():
                if word.endswith(("Error", "NotFound")):
                    continue
            script_parts.extend(
                [
                    "",
    ...
    def generate_report(self, report: InvestigationReport) -> str:
        ...
        for step in report.steps:
            ...
            lines.append(
                f"- {icon} [{step.step_type}] {step.target}: {step.findings or step.result[:80]}"
            )
        if report.root_cause:
