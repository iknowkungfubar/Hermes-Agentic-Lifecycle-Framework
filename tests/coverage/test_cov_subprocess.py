...
def cov_run_module(module_name: str, args: list[str] | None = None, **kw):
    ...
    return subprocess.run(
        cmd, capture_output=True, text=True, timeout=30, cwd=HERE, **kw
    )
...
""")
    ...
    def test_voice(self):
        r = cov_run_code(
            "from half.half_voice.engine import VoiceEngine; e = VoiceEngine(); print('OK')"
        )
        assert r.returncode == 0

    def test_security(self):
        r = cov_run_code(
            "from half.security_scanners import GarakScanner, BumblebeeScanner; print('OK')"
        )
        assert r.returncode == 0

    def test_browser(self):
        r = cov_run_code(
            "from half.browser_research import BrowserResearchAgent; a = BrowserResearchAgent(); print('OK')"
        )
        assert r.returncode == 0

    def test_rest_daemon(self):
        r = cov_run_code(
            "from half.rest_daemon import RESTAPIHandler, run_server; print('OK')"
        )
        assert r.returncode == 0

    def test_stale_monitor(self):
        r = cov_run_code(
            "from half.stale_monitor import StaleSessionMonitor; m = StaleSessionMonitor(); m.scan(); print('OK')"
        )
        assert r.returncode == 0
    ...
    def test_ralph_loop(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(
                ["git", "commit", "-q", "-m", "init"], cwd=tmp, capture_output=True
            )
            r = cov_run_code(f"""
import sys
sys.path.insert(0, '.')
