...
class TestDatabaseFull:
    def test_lease_flow(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            db.send_message(MessageType.TASK_ASSIGNMENT, "x@h.local", ["y@h.local"], "Review", "Please")
...
class TestNodesFull:
    def test_all_phase_functions_import(self):
        from half.runtime.nodes import (
            phase_1_discovery, phase_1_gate, phase_2_scaffold,
            phase_2_gate, phase_3_gate, phase_4_gate, phase_5_gate,
            route_from_gate
        )
    ...
    def test_route_from_gate(self):
        ...
        result = route_from_gate({"gate_1_passed": True, "phase_1_complete": True, "messages": []})
