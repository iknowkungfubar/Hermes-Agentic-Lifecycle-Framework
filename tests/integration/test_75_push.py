...
class TestHalfSidecarMissedLines:
    ...
    def test_main_dispatch(self):
        ...
        for cmd, maker in [
            ...
            (
                "run-phase",
                lambda: ns(command="run-phase", phase="phase-1", version=False),
            ),
            (
                "gate-check",
                lambda: ns(command="gate-check", phase="phase-1", version=False),
            ),
            ("generate-mrp", lambda: ns(command="generate-mrp", version=False)),
            (
                "init",
                lambda: ns(
                    command="init", project="p", mode="full", dir="/tmp", version=False
                ),
            ),
        ]:
