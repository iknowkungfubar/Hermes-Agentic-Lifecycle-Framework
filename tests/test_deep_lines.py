...
class TestMainDeepLines:
    ...
    def test_dispatch_version(self):
        ...
        assert (
            _route_command(argparse.Namespace(command="version", version=False)) is None
        )
    ...
    def test_dispatch_run_phase(self):
        ...
        r = _route_command(
            argparse.Namespace(command="run-phase", phase="phase-1", version=False)
        )
        assert isinstance(r, dict)
    ...
    def test_dispatch_gate_check(self):
        ...
        r = _route_command(
            argparse.Namespace(command="gate-check", phase="phase-1", version=False)
        )
        assert isinstance(r, dict)
    ...
    def test_dispatch_init(self):
        ...
        r = _route_command(
            argparse.Namespace(
                command="init", project="test", mode="full", dir="/tmp", version=False
            )
        )
