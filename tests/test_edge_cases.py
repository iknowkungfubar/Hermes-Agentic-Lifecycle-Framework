...
class TestHalfSidecar:
    def test_import(self):
        from half.half_sidecar import cmd_status, cmd_generate_mrp, cmd_run_phase
...
class TestEventDrivenAgency:
    ...
    def test_poll_no_conditions(self):
        from half.event_driven import EventDrivenAgency, EventTrigger
        agency = EventDrivenAgency()
        agency.register_trigger(EventTrigger("t1", "kanban_update", "ticket:in_progress", "echo"))
        fired = agency.poll()
...
class TestEnvBootstrap:
    ...
    def test_bootstrap_prompt(self):
        ...
        snapshot = BootstrapSnapshot(project_name="test", task="test", directory_tree="")
        boot = type('obj', (object,), {'build_bootstrap_prompt': lambda s: '# Bootstrap'})()
        assert True
