...
class TestRLVMR:
    def test_import(self):
        from half.rlvmr import RLVMRTracker, CognitiveStep, RLVMRRun
    ...
    def test_tag_step_planning(self):
        from half.rlvmr import RLVMRTracker, CognitiveStep
        tracker = RLVMRTracker()
        tracker.start_run("r1", "Task")
        tag = tracker.tag_step("r1", CognitiveStep.PLANNING, "plan approach", token_cost=100, success=True)
    ...
    def test_efficiency(self):
        ...
        tracker.tag_step("r3", CognitiveStep.PLANNING, "plan", token_cost=100, success=True)
    ...
    def test_best_strategy_with_data(self):
        ...
        tracker.tag_step("r5", CognitiveStep.PLANNING, "plan", token_cost=10, success=True)
        assert "Best run" in tracker.get_best_strategy()


class TestRouting:
    def test_import(self):
        from half.routing import TaskRouter, WorkflowType, RoutingDecision
...
class TestLMStudio:
    def test_import(self):
        from half.lm_studio import ModelConfig, InferenceProvider, LMStudioManager
...
class TestPydanticAI:
    ...
    def test_create_capability(self):
        ...
        cap = Capability(id="C-001", description="User authentication with JWT", priority="P0", confidence="HIGH")
        assert cap.id == "C-001"
...
class TestRestDaemon:
    def test_import(self):
        from half.rest_daemon import run_server, RESTAPIHandler
