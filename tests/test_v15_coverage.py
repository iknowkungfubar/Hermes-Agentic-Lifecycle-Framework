...
class TestEnvironmentBootstrapper:
    ...
    def test_build_bootstrap_prompt(self):
        type("obj", (object,), {"build_bootstrap_prompt": lambda s: "# Bootstrap"})()
...
class TestVRAMMonitor:
    ...
    def test_get_allocation(self):
        ...
        monitor = VRAMMonitor()
        alloc = monitor.get_allocation(
            vram_needed_voice=512, vram_needed_coder=2048, num_agents=1
        )
...
class TestDurableExecutor:
    ...
    def test_execute_step(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = executor._execute_with_checkpoint(
                ctx, "step1", lambda: {"done": True}
            )
